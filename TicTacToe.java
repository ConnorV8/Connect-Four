...
class TicTacToe extends AlternatingGame {
     ...
     * Build an array of all children of a given board
     */
    public static Object[] getChildren(Object aB){
	char[] aBoard = (char[])aB;
    
	// Build the children
	Object[] rv = new Object[getNumChildren(aB)];

	// Which player will be making the moves...
	char playerChar = playerOneCharacter;
	    if (getNumChildren(aBoard) % 2 == 0)
		playerChar = playerOneCharater;
	    if (getNumChildren(aBoard) % 2 == 1)
		playerChar = playerTwoCharacter;
	
	/* Your Homework Here!!! Use getNumChildren() method */
    ...
    public static int getNumChildren(Object aB){
	char[] aBoard = (char[])aB;
	int nc = 0;        // number of children
	for(int i = 1; i<= 9; i++){
	    if(aBoard[i] != playerOneCharacter &&
