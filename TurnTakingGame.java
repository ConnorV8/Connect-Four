...
abstract class TurnTakingGame {
    ...
    int evaluateTree(DSNode root){
        ...
        for(int i = 0; i < root.getNumChildren(); i++){
            evaluateTree(root.getChildren()[i]);  // Recursive call
            if(whoseTurn == 1){
                if(winner == 2)
                    winner = root.getChildren()[i].getWinner();
                else if(winner == 0 && 
                        root.getChildren()[i].getWinner() == 1)
                    winner = 1;
            } else {
                if(winner == 1)
                    winner = root.getChildren()[i].getWinner();
                else if(winner == 0 && 
                        root.getChildren()[i].getWinner() == 2)
                    winner = 2;
            }
    ...
    //    abstract int      getNumChildren(Object board);
